  ...
  return (
    <div className="space-y-8">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">إعداداتي</h3>
        {loading ? (
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              ...
              <div className="flex items-center justify-between"><span className="text-gray-600 dark:text-gray-400">الطول (سم)</span><span className="text-gray-900 dark:text-white">{(user as any).heightCm ?? (user as any).metadata?.heightCm ?? '-'}</span></div>
              <div className="flex items-center justify-between"><span className="text-gray-600 dark:text-gray-400">الوزن الابتدائي (كجم)</span><span className="text-gray-900 dark:text-white">{(user as any).baselineWeightKg ?? '-'}</span></div>
              <div className="flex items-center justify-between md:col-span-2"><span className="text-gray-600 dark:text-gray-400">ملاحظات صحية</span><span className="text-gray-900 dark:text-white">{(user as any).healthNotes || '-'}</span></div>
              <div className="flex items-center justify-between"><span className="text-gray-600 dark:text-gray-400">مستوى العضوية</span><span className="text-gray-900 dark:text-white">{user.membershipLevel}</span></div>
              <div className="flex items-center justify-between"><span className="text-gray-600 dark:text-gray-400">نقاط الولاء</span><span className="text-gray-900 dark:text-white">{user.loyaltyPoints}</span></div>
            </div>
            <div className="pt-2">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input name="heightCm" value={form.heightCm} onChange={handleChange} className="w-full px-3 py-2 rounded border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-white" />
                </div>
                <div>
                  <label className="block text-sm text-gray-700 dark:text-gray-300 mb-1">الوزن الابتدائي (كجم)</label>
                  <input name="baselineWeightKg" value={form.baselineWeightKg} onChange={handleChange} className="w-full px-3 py-2 rounded border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-white" />
                </div>
              </div>
              <div>
                <label className="block text-sm text-gray-700 dark:text-gray-300 mb-1">ملاحظات صحية</label>
                <textarea name="healthNotes" value={form.healthNotes} onChange={handleChange} className="w-full px-3 py-2 rounded border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-white" />
              </div>
