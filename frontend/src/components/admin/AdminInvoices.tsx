          ...
          <div className="bg-white dark:bg-gray-900 rounded-lg shadow-xl max-w-3xl w-full mx-4 p-6 relative animate-fade-in">
            ...
            <div className="space-y-3 text-sm">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                <div><span className="text-gray-500">رقم الفاتورة:</span> <span className="font-medium">{selectedInvoice.invoiceNumber}</span></div>
                <div><span className="text-gray-500">المستخدم:</span> <span className="font-medium">{userMap[selectedInvoice.userId]?.name || selectedInvoice.userId}</span></div>
                <div><span className="text-gray-500">الهاتف:</span> <span className="font-medium">{userMap[selectedInvoice.userId]?.phone || '-'}</span></div>
                <div><span className="text-gray-500">القيمة:</span> <span className="font-medium">{fmt(selectedInvoice.amount)}</span></div>
                <div><span className="text-gray-500">مدفوع:</span> <span className="font-medium">{fmt(selectedInvoice.paidAmount || 0)}</span></div>
                <div><span className="text-gray-500">الحالة:</span> <span className="font-medium">{selectedInvoice.status}</span></div>
                <div><span className="text-gray-500">الإصدار:</span> <span className="font-medium">{String(selectedInvoice.issueDate).slice(0,10)}</span></div>
                <div><span className="text-gray-500">الاستحقاق:</span> <span className="font-medium">{selectedInvoice.dueDate ? String(selectedInvoice.dueDate).slice(0,10) : '-'}</span></div>
              </div>
              <div>
                  ...
                  <tbody>
                    {(selectedInvoice.items || []).map((it, i) => (
                      <tr key={i} className="border-t border-gray-200 dark:border-gray-700">
                        ...
                        <td className="px-2 py-1">{fmt(it.price)}</td>
                        <td className="px-2 py-1">{fmt((it.quantity || 0) * (it.price || 0))}</td>
                      </tr>
                    ))}
                    {(!selectedInvoice.items || selectedInvoice.items.length === 0) && (
                      <tr>
                        <td className="px-2 py-3 text-center text-gray-500" colSpan={4}>لا توجد عناصر</td>
                      </tr>
                    )}
                  </tbody>
                </table>
            ...
            <div className="flex justify-end gap-2 mt-6">
              <button className="px-4 py-2 bg-gray-500 text-white rounded hover:bg-gray-600" onClick={() => printInvoice(selectedInvoice)}>طباعة الفاتورة</button>
              <button className="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700 disabled:opacity-50" disabled={updatingId === selectedInvoice._id} onClick={() => handleDeleteInvoice(selectedInvoice._id)}>{updatingId === selectedInvoice._id ? 'جارٍ الحذف...' : 'حذف الفاتورة'}</button>
              <button className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700" onClick={() => openEditModal(selectedInvoice)}>تعديل الفاتورة</button>
            </div>
          </div>
        </div>
      )}
